...
def public_state(room, player_id=None):
    ...
    if player_id:
        ...
        if student:
            pred = conn.execute(
                "SELECT choice FROM predictions "
                "WHERE room=? AND round_no=? AND player_id=?",
...
def create_game():
    ...
    total_rounds = max(2, min(total_rounds, 100))
    ...
    with db_lock:
        ...
        while True:
            ...
            if not conn.execute(
                "SELECT 1 FROM games WHERE room=?", (room,)
            ).fetchone():
                break
...
def join_game():
    ...
    if not room or not name or not player_id:
        return jsonify({
            "ok": False,
            "error": "Room, name, and player ID are required."
        }), 400
...
def lock_and_roll():
    ...
    with db_lock:
        ...
        if game["status"] != "accepting":
            ...
            return jsonify({
                "ok": False,
                "error": "This round has already been locked or rolled."
            }), 400
        ...
        conn.execute(
            "UPDATE games SET status='result' WHERE room=?",
            (room,)
        )
...
def next_round():
    ...
    with db_lock:
        ...
        if game["round_no"] >= game["total_rounds"]:
            ...
            conn.execute(
                "UPDATE games SET round_no=round_no+1, status='accepting' "
                "WHERE room=?",
                (room,)
            )
...
def export_csv():
    ...
    return Response(
        ...
        headers={
            "Content-Disposition":
                f"attachment; filename=stochastic_challenge_{room}.csv"
        },
    )
